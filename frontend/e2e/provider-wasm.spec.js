...
test('executes the selected WASM parser path', async ({ page }) => {
  ...
  await page.goto('/?tryOnParser=wasm', { waitUntil: 'networkidle' });
  ...
  await expect(dialog).toHaveAttribute('data-parser-provider', 'wasm');
  await expect.poll(
